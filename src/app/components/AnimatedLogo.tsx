...
const AnimatedLogo = ({ size = 200, className = '' }: { size?: number; className?: string }) => {
  return (
    <div className={`relative flex items-center justify-center ${className}`} style={{ width: size, height: size }}>
      {/* Satellite - orbits into position (0-1s) */}
      <motion.div
      ...
      >
        <Satellite size={size * 0.2} className="text-blue-500" />
      </motion.div>

      {/* Signal waves - pulse outward (1-2s) then continuous loop */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      {/* Hexagonal grid - draws onto leaf (2-3s) */}
      <motion.div
      ...
      >
        ...
        >
          ...
          />
          <motion.path
            d="M50 5 L50 95 M10 25 L90 75 M10 75 L90 25"
            fill="none"
            stroke="currentColor"
            strokeWidth="0.5"
            initial={{ pathLength: 0 }}
      ...
      {/* KhetMap text - fades in with slide-up (4-5s) */}
      <motion.div
